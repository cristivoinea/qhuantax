...
import numpy as np
from qhuantax.quantumhall_operators import GetSpinfulDenIntTerms, GetSpinfulPolTerms, GetLpTerms, GetIdTerms
...
if do_ED:
    ...
    exact_LmLp = qtx.utils.DataTracer()
    exact_variance = qtx.utils.DataTracer()
    #dense_H = tms_H.todense(symm)
    #dense_LmLp = tms_LmLp.todense(symm)
    exact_L0proj = qtx.utils.DataTracer()

    def OneMinusProjector(L_target, L_proj):
        return (tms_LmLp - L_proj*(L_proj+1)*tms_id/N)/(L_target*(L_target+1) - L_proj*(L_proj+1))
